 ...
 * 
 * Refactored by SPP DEV MDic for Trinitycore 335
 * original work and source by PARRANOIA
 * https://pastebin.com/ZGFNWaVt
 */
...
public:
    ...
    {
        if (sConfigMgr->GetBoolDefault("Auto.Learn.enable", true) && oldLevel < player->GetLevel())
    ...
    {
        if (sConfigMgr->GetBoolDefault("Auto.Learn.enable", true) && level == player->GetLevel() + 1)
            return;
        uint32 family;

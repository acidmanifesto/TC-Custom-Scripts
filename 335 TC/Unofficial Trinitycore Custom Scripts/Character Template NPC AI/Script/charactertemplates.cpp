...
void sTemplateNPC::LearnTemplateTalents(Player* player)
{
    for (TalentContainer::const_iterator itr = m_TalentContainer.begin(); itr != m_TalentContainer.end(); ++itr)
    {
        if ((*itr)->playerClass == GetClassString(player).c_str() && (*itr)->playerSpec == sTalentsSpec)
        {
            ...
            player->AddTalent((*itr)->talentId, player->GetActiveSpec(), 0);
        }
    }
...
void sTemplateNPC::LoadTalentsContainer()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded 0 talent templates. DB table `template_npc_talents` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded %u talent templates in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void sTemplateNPC::LoadGlyphsContainer()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded 0 glyph templates. DB table `template_npc_glyphs` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded %u glyph templates in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void sTemplateNPC::LoadHumanGearContainer()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded 0 'gear templates. DB table `template_npc_human` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded %u gear templates for Humans in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void sTemplateNPC::LoadAllianceGearContainer()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded 0 'gear templates. DB table `template_npc_alliance` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded %u gear templates for Alliances in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void sTemplateNPC::LoadHordeGearContainer()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded 0 'gear templates. DB table `template_npc_horde` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">>TEMPLATE NPC: Loaded %u gear templates for Hordes in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
public:
    ...
    struct TemplateNPC_AI : public ScriptedAI
    {
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_TRAINER, "Menu", GOSSIP_SENDER_MAIN, 11111);
        ...
        bool OnGossipSelect(Player* player, Creature* _creature, uint32 sender, uint32 uiAction)
        {
            ...
            case 11111:
                ...
                if (player)
                {
                    ...
                    SendGossipMenuFor(player, 55002, _creature->GetGUID());
                    break;
                }
                else
                {
            ...
            default: // Just in case custom class is used
...
public:
    ...
    ChatCommandTable GetCommands() const override
    {
        ...
        {
            { "reload", HandleReloadTemplateNPCCommand, rbac::RBAC_ROLE_ADMINISTRATOR, Console::Yes}
        };
